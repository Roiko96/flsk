...
from functions import add_score_data, delete_score_by_name, sort_scores_by_key, calculate_average, edit_score_data, get_scores
import os, re, time
...
def index():
    ...
    if sort_key:
        sort_scores_by_key(sort_key)
        if sort_key == "score" and order == "asc":
            sort_scores_by_key("name")
...
def delete():
    name = request.form.get("name","").strip()
    if name: delete_score_by_name(name)
    return redirect(url_for("index", t=int(time.time()*1000)))
