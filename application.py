...
def inject_host():
    return {"host": os.getenv("HOSTNAME","ec2")}
...
def index():
    ...
    view = get_sorted_scores(sort_key, reverse=(order=="desc")) if sort_key in allowed else get_scores()
...
def sort_view(key):
    key = key.lower()
    if key not in {"name","game","score"}: return redirect(url_for("index"))
